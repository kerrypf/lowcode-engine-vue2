...
export function createWatcher(
  ...
  if (isString(raw)) {
    ...
    if (isFunction(handler)) {
      ...
      console.warn(`Invalid watch handler specified by key "${raw}"`, handler);
    }
    ...
    if (isArray(raw)) {
      ...
      if (isFunction(handler)) {
        ...
        console.warn(`Invalid watch handler specified by key "${raw.handler}"`, handler);
      }
    }
  } else {
    // warn(`Invalid watch option: "${key}"`, raw);
    console.warn(`Invalid watch option: "${key}"`, raw);
  }
}
